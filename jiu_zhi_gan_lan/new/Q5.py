    ...
    for _ in range(0, 1):
        for distance in distances:
            ...
            for j in range(len(init_v_list)):
                ...
                if time_ > min(best_times):
                    idx = best_times.index(min(best_times))
                    best_times[idx] = time_
                    best_vs[idx] = init_v
                    best_ps[idx] = init_p
                    best_ts[idx] = init_t
                    best_ms[idx] = m
                    best_bs[idx] = distance


    return best_times, best_vs, best_ps, best_ts, best_ms, best_bs
best_times, best_vs, best_ps, best_ts, best_ms, best_bs = find_best_params([m1, m2, m3], fy1_pos, target_true_pos, [0, 3000], 100)
for i in range(3):
    print(f"Best Time: {best_times[i]}, Best V: {best_vs[i]}, Best P: {best_ps[i]}, Best T: {best_ts[i]}, Best M: {best_ms[i](0)}, Best B:{best_bs[i]}")
class Optimization:
    def __init__(self):
        self.history = []
        self.params_history = []
